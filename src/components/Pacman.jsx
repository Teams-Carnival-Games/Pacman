...
import PacmanService from "../services/fluidService";
...
function Pacman() {
  ...
  const fetchTeamsUserName = useCallback(async () => {
    ...
    if (isLoaded) { // Only send the message if Unity is ready
...
useEffect(() => {
  const handleHealthUpdate = (playerName, updatedHealthData) => {
      console.log(`Received health update for ${playerName}: `, updatedHealthData);
...
useEffect(() => {
  PacmanService.onHealthUpdate((playerName, updatedHealthData) => {
      ...
      // Send updated health data to Unity
                sendMessage("Dataexporter", "ReceiveHealthData", JSON.stringify(updatedHealthData));
  });
}, []);
...
useEffect(() => {
  ...
  const processReportedScore = useCallback(async (scoreData) => {
    ...
    await PacmanService.updateScoresFromUnity({ [playerName]: parseInt(playerScore) });
  }, []);
...
const triggerDownload = (jsonData) => {
  ...
  useEffect(() => {
    const handleScoreUpdate = (playerName, newScore) => {
        ...
        sendMessage("ScoreManager", "NewDataReceivedFromReact", JSON.stringify({type: "ScoreUpdate", playerName, newScore}));
    ...
    const initializeAppAndFetchData = async () => {
        try {
            ...
            if (await PacmanService.hasContent()) {
                ...
                if (isLoaded) {
                    const wrappedScores = { type: "ScoreContainer", scores: initialScores };
                    console.log("being sent to unity from fluid!:", JSON.stringify(initialScores));
  ...
  return (
    <Fragment>
      <div style={{ display: "flex", alignItems: "flex-start", height: "100vh", overflow: "hidden", width: unityWidth }}>
   <Unity unityProvider={unityProvider} style={{ width: unityWidth + 40, height: unityHeight, marginLeft: '-20px' }} />
</div>
