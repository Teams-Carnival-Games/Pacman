...
var ASM_CONSTS = {
  3046548: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 3046603: function($0) {performance.now = function() { return $0; };},  
 3046651: function($0) {performance.now = function() { return $0; };},  
 3046699: function() {performance.now = Module['emscripten_get_now_backup'];},  
 3046754: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 3046815: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 3046879: function() {return Module.webglContextAttributes.powerPreference;}
};
  ...
  function _DownloadHealthData(dataPtr) {
          try {
              console.log("DownloadHealthData function entered.");  // Debug: function start
  
              let jsonData = Pointer_stringify(dataPtr);
              console.log("DownloadHealthData received: ", jsonData);  // Existing log statement
  
              console.log("Dispatching event to React...");  // Debug: before dispatching the event
              window.dispatchReactUnityEvent("DownloadHealthData", jsonData);
              console.log("Event dispatched to React successfully.");  // Debug: after dispatching the event
  
          } catch (e) {
              console.warn("Failed to download HealthData", e);
          }
      }
  ...
  function _ReportScoreToReact(scoreDataPtr) {
      try {
        let scoreData = Pointer_stringify(scoreDataPtr);  // Convert the pointer to a JS string
        window.dispatchReactUnityEvent("ReportScore", scoreData);
      } catch (e) {
        console.warn("Failed to dispatch ReportScore event", e);
      }
    }

  function _UpdateReactWithHealthData(usernamePtr, frequency, duration) {
          try {
              let username = Pointer_stringify(usernamePtr); 
              let healthData = JSON.stringify({ username, frequency, duration });
              window.dispatchReactUnityEvent("UpdateHealthData", healthData);
          } catch (e) {
              console.warn("Failed to update React with HealthData", e);
          }
      }
...
var asmLibraryArg = {
  "DownloadHealthData": _DownloadHealthData,
  ...
  "ReportScoreToReact": _ReportScoreToReact,
  "UpdateReactWithHealthData": _UpdateReactWithHealthData,
